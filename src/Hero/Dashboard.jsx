...
import { addMood, updateMood } from "../services/MoodTracker/MoodThunk";
...
export default function Dashboard() {
  ...
  const dateStr = dayKey(year, month, day);
  ...
  if (existingMood) {
    dispatch(updateMood({ id: existingMood.id, moodKey: activeMood }));
  } else {
    dispatch(addMood({ date: dateStr, moodKey: activeMood }));
  }
  ...
  return (
    <div style={{
      ...
      <Sidebar active="Home" bgc="#FFF0F6EB" isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} />
      ...
      >
            ...
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", alignItems: "center" }}>
              {QUICK_MOODS.map((m) => (
                <button key={m.key}
                  ...
                  style={{
                    fontSize: "1.5rem",
                    background: activeMood === m.key ? "linear-gradient(135deg, rgba(252,231,243,0.95), rgba(243,232,255,0.9))" : "rgba(255,255,255,0.7)",
                    border: activeMood === m.key ? "2px solid #ec4899" : "2px solid transparent",
                    borderRadius: "0.75rem",
                    width: "2.6rem", height: "2.6rem",
                    cursor: "pointer",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    transition: "all 0.2s",
                    transform: activeMood === m.key ? "scale(1.2)" : "scale(1)",
                    boxShadow: activeMood === m.key ? "0 4px 12px rgba(236,72,153,0.25)" : "none",
                  }}
                  onMouseOver={(e) => { if (activeMood !== m.key) e.currentTarget.style.transform = "scale(1.1)"; }}
                  onMouseOut={(e) => { if (activeMood !== m.key) e.currentTarget.style.transform = "scale(1)"; }}
                >
                  {m.emoji}
                </button>
              ))}
        ...
        <div style={{ animation: "fadeUp 0.6s 0.3s ease both", opacity: 0 }}>
